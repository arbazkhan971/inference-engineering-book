...
> **Part IV — Harness meets engine** — chapter 16 gave you a fleet and a meter. Every request your harness sends is a cache key before it is a conversation (the bytes themselves are what the provider files it under), and the session — not the request — is where caches are won or lost.
...
- **Layer 1 — tools.** Frozen, versioned, deterministic serialization order. Chapter 13's frozen-schema discipline lives here: identical schema bytes at the head, every call. Tool churn goes through deferred loading — placeholder entries plus on-demand tool search — rather than add/remove cycles that re-write the whole prefix on every reconnect.
...
2. **Pin per session.** A session carries the template version it was born with; new sessions get the new bytes, live ones keep the old until they end. The herd event — every live session going cold at once, chapter 6's *cache-herd event* — becomes a staggered roll.
...
The courier office, the keycard, the handbook, the accountant — each earns its keep, and each breaks somewhere specific:
...
**The accountant counts events, not meaning.** The ledger prices every cache event and judges none of them — a ledger row cannot tell you the summary was wrong. Quality is the one cost the meter never sees; chapter 11's benchmarks own that question, not the books.
...
4. Ten subagents of one type; each needs a 20,000-token preamble and carries a 6,000-token task. Shared spawn template vs. each child embedding its own copy of the preamble in its head: compare in uncached-equivalent units. *(Shared: the first child pays 1.25·20K to write the preamble + 6K fresh; the other nine pay 0.1·20K read + 6K fresh each: 25K + 6K + 9 × 8K = 103K-units. Isolated: 10 × 1.25 × 26K = 325K-units. The fleet shape is ~3.2× cheaper — derived; on Anthropic, stagger the first child's write before the other nine fire.)*
...
tinyengine's `SessionStore` — the component the whole chapter has been drafting. Four parts: an *append-only event log* (session creation, message append, compaction, and breakpoint events, each versioned and globally sequenced); a *renderer* that serializes the five layers deterministically (tools → system → static context → transcript → volatile tail) with the template version and tool-key order pinned at session start, and chapter 14's breakpoint placement, leapfrog included; a *TTL policy engine* that classifies sessions by idle pattern (interactive / think-time / overnight), requests 5-minute or 1-hour entries accordingly, schedules keep-alive ticks through chapter 15's scheduler gate, and prices lifecycle events onto chapter 14's `CacheLedger`; and a `spawn(template, tasks, parent)` path that renders shared-preamble children with breakpoints on the template's frozen layers and staggers the fleet behind the first child's write.

Persistence is an explicit seam, not a hidden promise. The default `MemorySessionEventStore` keeps lightweight callers and unit tests self-contained; inject `JsonlSessionEventStore` when a session must survive a process restart. It writes one complete JSON object per line before mutating the in-memory projection, validates version, shape, content hash, and sequence during replay, and fails loudly on middle corruption or a sequence gap. A final line without its commit newline is never appendable: if its JSON object is complete, replay exposes it read-only; if the crash tore the object itself, replay drops only that fragment and preserves the earlier events. Repair the tail before writing again. Compaction appends a summary event and replaces only the active prompt projection: `history()` and the durable log retain every original message.

### Break it

Three injections, one per subsystem. (1) *The lying serializer:* patch the renderer to emit tool keys in hash-map order; a five-turn scripted session should show the cache-read share collapsing in the usage events — and the CI test (below) should fail first. (2) *The TTL misclassification:* classify a session that idles six minutes as interactive; the ledger should emit a cold-rebuild cost event (write where a read was budgeted), proving the idle classifier is a money decision. (3) *The chatty child:* spawn a subagent that returns raw tool debris into the parent transcript; the parent's per-turn prefix cost should jump visibly in the ledger — garbage made permanent, visible only in the meter.

### Prove it

Byte-exactness as a restart test, not a same-process hope: write a session to JSONL, render and hash it, construct a second `SessionStore` over that file, render and hash again, and assert equality. Compact the session, then prove the resumed prompt contains only the summary plus later turns while `history()` still contains the pre-compaction messages. Corrupt a middle record and create a sequence gap; both must fail closed. Tear the final JSON object and prove replay recovers the complete prefix but rejects the next append; separately remove only the final newline and prove the complete object remains readable but still not appendable. Then add a golden-file pass: a fixed session rendered across library versions must produce identical bytes or fail the build. Finally a live five-turn scripted run against a real provider can assert the cached-token share stays above a threshold across turns 2–5, and that a forced 10-minute idle produces exactly one rebuild write, sized to the prefix.

### See it in the wild

Anthropic's prompt-caching docs and the Claude Code prompt-caching page are the deepest public treatment of session-scale cache engineering — the TTL clock from request start, the four-layer stack, the TTL buckets per conversation type, the explicit list of which actions keep the cache; Claude Code's sessions docs and issues #42338/#71659 document resume pain in the operator's own words. OpenAI's prompt-caching guide carries the fork guidance, the per-machine routing caveat, and the `prompt_cache_key` fix; Anthropic's Compaction API page productizes the rewrite; MemGPT (arXiv 2310.08560, 2023) and Letta's core-memory docs own the external-memory half; TokenPilot (arXiv 2606.17016, June 2026) names the sparsity-versus-continuity tension; the lost-in-compaction benchmark (Zenodo DOI 10.5281/zenodo.20273814) prices the information half. Then look at your own product's longest-lived session and ask the ledger's questions: what is frozen, what does a gap cost, and what did the last deploy do to the fleet?
