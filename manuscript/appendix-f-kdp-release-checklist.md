...
| Companion attack (gate-6, 2026-08-28) | P1 pass applied (7/7); 10 P2s queued | 14 targeted attacks against the companion's money and admission paths (review/gate6-companion-attack.md + tests/attack-gate6.ts): 0 P0s, 7 P1 + 11 P2. All 7 P1s applied 2026-08-28 — `data:null`/array/string payloads skip instead of crashing the stream loop; non-numeric usage fields coerce to 0 with an `incomplete` flag instead of poisoning the meter with NaN; reservations are atomic (a TPM miss no longer burns the RPM slot); Bedrock overruns (output × burndown > booked) are debited, with the both-ways reconcile added to ch15's Build-it and Appendix D; an unknown model throws a named `UnknownModelError` before any session state mutates; `CacheLedger.hitRate` aligned to the book's cached ÷ (cached + fresh), agreeing with cache-hit-gate by construction; duplicate invoice and meter rows sum instead of last-wins (the gap-sign inversion fixed; the meter-side aggregation also resolved E2, one of the P2s, leaving 10). Key repros wired into tests/smoke.ts and tests/cadence.ts; attack-gate6.ts kept as regression evidence (its C1 case reports by construction on any throw — the named error it asked for — so its regression gate lives in smoke). The remaining 10 P2s are now the last queue before re-seal (the wave-1 P2 pass landed — row above) |
