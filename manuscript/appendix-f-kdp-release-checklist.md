...
| Adversarial claims + xref audit (gate-6 wave 1, 2026-08-28) | P1 pass applied; 11 P2s queued | Independent re-derivations + ~660 pointer checks (review/gate6-*.md): 0 P0s, 5 P1 + 11 P2. All 5 P1s applied 2026-08-28 — ch03's community 4-bit throughput relabeled batched aggregate with the derived 50–57 tokens/s single-stream ceiling; ch04's Anthropic tier row/prose aligned to the 4.6+ standard-price fact (older-sheet tier kept, dated); SchemaGuard de-attributed from the companion (ch13 Build-it, ch16 routing table, with the deliberate-omission note in Appendix D); ch12/ch18 event counts restated as four streaming events plus two finish-time markers (figure 12-m1 relabeled + re-rendered); the usage ledger gained its cache-write bucket (reasoning now a billed sub-count, matching the code's five fields and the four priced buckets everywhere else). The wave-1 P2s (claims-1/2/3 + xref) remain queued; the iteration-54 seal re-opens until they are applied |
| Companion attack (gate-6, 2026-08-28) | P1 pass applied (7/7); 10 P2s queued | 14 targeted attacks against the companion's money and admission paths (review/gate6-companion-attack.md + tests/attack-gate6.ts): 0 P0s, 7 P1 + 11 P2. All 7 P1s applied 2026-08-28 — `data:null`/array/string payloads skip instead of crashing the stream loop; non-numeric usage fields coerce to 0 with an `incomplete` flag instead of poisoning the meter with NaN; reservations are atomic (a TPM miss no longer burns the RPM slot); Bedrock overruns (output × burndown > booked) are debited, with the both-ways reconcile added to ch15's Build-it and Appendix D; an unknown model throws a named `UnknownModelError` before any session state mutates; `CacheLedger.hitRate` aligned to the book's cached ÷ (cached + fresh), agreeing with cache-hit-gate by construction; duplicate invoice and meter rows sum instead of last-wins (the gap-sign inversion fixed; the meter-side aggregation also resolved E2, one of the P2s, leaving 10). Key repros wired into tests/smoke.ts and tests/cadence.ts; attack-gate6.ts kept as regression evidence (its C1 case reports by construction on any throw — the named error it asked for — so its regression gate lives in smoke). The remaining 10 P2s stay queued with the wave-1 eleven |
...
The copyedit pass closed 2026-08-27. The gate-6 adversarial wave (claims falsifiers + cross-reference audit) landed 2026-08-28: its five P1s are applied and its eleven P2s are queued. The gate-6 companion attack (same day) found seven P1s in the money and admission paths — all seven are applied, with key repros wired into the companion suites; ten of its P2s remain queued (the meter-side duplicate fix resolved the eleventh). The release seal re-opens until both P2 passes land. After that, final proof (Kindle Previewer) is the one remaining gate before upload.
