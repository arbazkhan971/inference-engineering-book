...
5. **Reflow.** A known residual: the prose contains ~54 overlong lines in 10 files (worst offenders in the streaming chapter), and the reflow checker currently freezes only with a raised budget. Release requires a decision — fix the lines or consciously ratchet the budget and record it here. Do not upload with the decision unmade.
6. **Kindle Previewer.** The one check no script performs: open the final EPUB in Kindle Previewer (phone and e-reader profiles) and page through front matter, every chapter opener, the dated boxes, and the appendices. This is the final-proof gate the editorial system deliberately leaves conditional.
