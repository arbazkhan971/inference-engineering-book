...
Goodput — completions per second that honor your SLO bounds, the metric chapter 5 defined — returns throughout; treat that table row as read.
...
3. **Queueing multiplies it all.** Chapter 5's M/G/1 arithmetic — mean wait ∝ 1/(1−ρ), with ρ the utilization — means a 15% load increase near saturation can multiply queue wait several-fold, and the p99 (99th percentile) diverges faster than the mean. Tail TTFT is a property of the *load*, not of your request.
