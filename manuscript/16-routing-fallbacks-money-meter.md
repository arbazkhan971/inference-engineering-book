...
- **Weighted routing.** Deployments of the same model carry `weight` fields; traffic splits by weight. Strategies range from `simple-shuffle` (recommended; weighted random) to `least-busy` (pick the idlest wire), `latency-based-routing` (pick the fastest), and `usage-based-routing` (by cost or by tokens/requests-per-minute usage).
...
One more input the resilience canon never had: **headroom telemetry.** Chapter 12 established that no hosted provider publishes time-to-first-token (TTFT) percentiles — your latency-based routing can only run on numbers you measure client-side. The same client-side layer that timestamps first tokens can read the small notes providers staple to every response — the `x-ratelimit-remaining-*` headers (chapter 15) — and a remaining-tokens gauge per endpoint is the earliest signal of a 429 wall — which turns "route to whoever is healthy" into "route to whoever still has headroom," minutes before the error rate says so.
...
Worked arithmetic on dated list prices (retrieved 2026-08-27): a 1-million-token eval suite, 800K input and 200K output, on GPT-4o at $2.50/$10.00 per million tokens — $0.8 × $2.50 + $0.2 × $10.00 = **$4.00** interactive, **$2.00** batch. The same suite on Claude Sonnet 4.6 at $3/$15 — **$5.40** interactive, **$2.70** batch. A nightly regression matrix of 50 such runs saves **$100–$135 per night** for identical answers, and at roughly 1,000 tokens (~4 KB) per request the whole matrix sits at OpenAI's batch-file ceilings (50,000 requests / 200 MB) — split it across two jobs rather than trust the fit. The answers are identical; only the arrival time changed.
...
> **ELI5:** One electricity meter for the whole building tells you the total and nothing else. Add a submeter per apartment and the bill becomes an *explanation*: unit 4 runs the dryer at night, unit 2 is away. Token metering is the same upgrade — from "we spent $9,000 on LLMs" to "the summarizer spent $2,100 — most of it surcharges for re-sending text it had already seen."

Chapter 2 promised this arithmetic: cost ≈ input_tokens × input_price + N × output_price — a straight-line (affine) formula: one term you control, one you rent. The money meter is that identity with real invoices bolted on, and real invoices discount. The correct general form is:
...
| Latency (from ch. 15) | wave-paced; waits on the slowest child of the 10,000 | ≤ 24 h; most jobs < 1 h | same |
...
**The lane beats the cache.** Interactive *with a perfect hit rate* (≈ $79.50 — the same worksheet with reads at $0.30/M and writes at $3.75/M — 0.1× and 1.25× of the $3 base, derived) still costs more than batch *with no cache at all* ($60.00). Mode choice is a bigger multiplier than prefix engineering; do it first, then optimize prefixes inside the lane. And the hit rate inside a batch window is *not yours to design* — no provider documents intra-batch cache behavior (section 16.4), so the hit rate, h, is measured, not assumed. The full cost curve, derived:
...
tinyengine's `Router` — the component chapter 15's scheduler was built to feed. Four parts: a *routing table* (alias → deployments with weights, guarantee tier (chapter 13's ladder), task tags, lane interactive/batch/flex, and a session-pinning rule that survives until a fallback breaks it — recording the break as a cache event on chapter 14's CacheLedger); a *breaker per deployment* with error-class-aware trips (429: short bench; 401/404: permanent bench pending human action; sustained failures: more than the configured fraction of a rolling 60-second sample window, after its minimum-sample floor) and exactly one leased half-open probe; a *fallback executor* that starts from the same weighted or pinned choice the router reported, then walks the remaining deployments only on classified errors — malformed 400s and garbage 200s go to the validator path, not the chain; and an attributed *route receipt* on every success. Your harness resolves task, lane, and guarantee policy into an alias; within that alias, this router chooses by pin, weight, and breaker state, then passes the chosen deployment's guarantee tier to the validator. A pin is keyed by alias plus session and commits only after a successful, validated response: a failed first choice cannot poison later turns, and one session can pin independent routes.

That receipt names `alias`, `deploymentId`, `model`, `provider`, `priceTableVersion`, `selectedBy`, `attempt`, and `at`. `selectedBy` distinguishes `weighted`, `pin`, `fallback`, and the loud `all_open_bypass`, so the money meter can explain exactly which route served the call and which dated prices apply. The bypass applies only to finite cooldowns; if every route is permanently open for authentication or not-found errors, the router stops and logs `ALL DEPLOYMENTS REQUIRE HUMAN ACTION`. Chapter 18's assembled path preserves the receipt beside normalized usage and cost.

### Break it

Three injections, one per component. (1) *The dead primary:* mock the first deployment to fail every call; verify traffic moves to the second deployment, the breaker benches the first, and — after the cooldown — exactly one concurrent caller receives the probe lease. An unrelated old success must not close the breaker. (2) *The garbage 200:* mock a 200 with a schema-violating body; verify the router does *not* fall back (it was not a classified error) and the validator-retry path from chapter 13 catches it instead — the failure that fallbacks cannot see. (3) *The all-open split:* put every deployment into a finite overload cooldown and verify one loud last-resort bypass executes; repeat with permanent 401/404 breakers and verify no call leaves the process. "Try anyway" is resilience during a transient incident, not a license to hammer invalid credentials.

### Prove it

Prove routing and money separately, then join them by receipt. Force weighted selection with an injected random value, make the first deployment fail with a classified status, and assert the fallback receipt names the second deployment, attempt 2, selection mode `fallback`, and the expected price-table version. Repeat with a session pin. Open every breaker with finite overloads and assert the last-resort call is labeled `all_open_bypass`; open them with auth errors and assert the router returns no response and emits the human-action log. For money, replay a fixture set of usage objects — one OpenAI-inclusive, one Anthropic-exclusive, one reasoning-heavy — through your four-bucket normalizer and assert exact costs against a hand-computed worksheet; any mismatch is a normalization bug, and the inclusive fixture will find it. Finally, run one real 3-request batch job end-to-end and check that usage lands *only* at completion, that errored requests are unbilled, and that the pending-jobs column absorbs the delay in daily reconciliation.

### See it in the wild

LiteLLM's proxy docs — reliability, load balancing, health-check-driven routing — are the open configuration surface for nearly every pattern in this chapter; Portkey's circuit-breaker page documents the count-or-rate trips and the all-open bypass; OpenRouter's reliability blog and model-fallbacks docs describe the two-layer failover, price-weighted selection, and the classified-error rule the Break-it injection exploits; Cloudflare's AI Gateway Dynamic Routing documents the `cf-aig-step` header. RouteLLM (arXiv 2406.18665, LMSYS blog 2024-07-01) is the complexity-routing reference with the honest three-benchmark savings spread; Martin Fowler's CircuitBreaker bliki and the Resilience4j docs carry the canonical state machine; Dean and Barroso's "The Tail at Scale" (Communications of the ACM, 2013) owns the fanout tail law the worksheet's latency row inherits. For the meter: Langfuse's model-usage-and-cost docs (the inclusive-to-exclusive normalization cliff, stated plainly) and OpenMeter's events/subjects/meters model — and every provider's invoice, the one dashboard this chapter insists you read daily.
