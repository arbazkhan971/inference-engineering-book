...
>
> **AWS Bedrock** — per-model token quotas: `input + cache-write tokens + (output tokens × burndown rate)`. Burndown multipliers: 15× (Claude 4.8), 10× (Claude Sonnet 5 / Opus 5), 5× (Claude 4.7 and below), 10× (GPT-5.6 Sol/Terra/Luna), 1:1 for most others. Cache-read tokens are not counted at all.
...
A rejection you can retry and a rejection you cannot look identical from a distance: both are errors, both arrive mid-fanout, both get caught by the same `except` clause. Treating them identically is the most expensive habit in this chapter.
...
OpenAI's 429 is deliberately overloaded in meaning: the same status can be a temporary rate limit, an exhausted prepaid balance, or a spend/usage limit — the docs instruct you to read the error's `code` and `type` fields, where billing-class failures surface as `insufficient_quota`, before you retry anything. Since July 2026, enforced monthly spend limits make this concrete: crossing the cap fails live requests with a 429 that no amount of retrying will fix. When rate limiting *is* the cause, OpenAI's responses include a `Retry-After` header when one is computable, plus `x-ratelimit-*` headers showing requests and tokens remaining and reset times — a dashboard your scheduler should read, not discard.
...
One number belongs here because it bounds everything in the next section: with a 3-attempt cap (the standard guidance), one logical request becomes at most 3 wire requests — the commonly cited 3× worst-case load amplification. Without a cap, there is no bound at all.
...
| Attempt cap 2–4 + ~10% retry budget | Bounds amplification at ~3× | Chapter 10's fleet rule |
