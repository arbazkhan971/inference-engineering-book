...
tinyengine's `RateScheduler` is a composition rather than one opaque class. Its *per-provider quota ledger* encodes each meter from section 15.3: OpenAI reserves `max(max_tokens, estimate)`; Anthropic exempts cache reads from ITPM and charges actual output to its own OTPM bucket, carrying an overrun as negative balance so later admissions wait for refill; Bedrock books `input + cache-write + max_tokens`, then reconciles the stored reservation against actual usage and the configured burndown multiplier. Every reservation receives an id. Completion settles that exact id, so out-of-order responses cannot credit the wrong request, and a duplicate or never-booked completion is a no-op. Under-runs re-credit; over-runs debit, because output × burndown can outrun the reservation and a ledger that only credits lets overrunners pass with the meter green.

The remaining parts are a *client token bucket* plus a Little's-Law-sized semaphore holding excess work in a visible local queue, a *retry module* — classifier first (billing 429s fail fast), `Retry-After` as floor, full jitter above it, 3-attempt cap, and a fleet-wide 10% retry budget that rejects surplus retries locally — and a *wave pacer* that spreads fanouts across time with jittered spacing and a K-of-N completion contract. Semaphore callers can use a one-use permit or `withPermit()`: a duplicate release cannot over-admit work. Chapter 16's router consumes the quota decision, and chapter 18 assembles the parts on the real request path.
...
Measure your provider's accounting, don't trust this chapter's snapshot. Send a fixed small workload and log the `x-ratelimit-*` response headers (OpenAI) after each call — remaining requests and tokens, reset times. Then change one variable at a time: raise `max_tokens` 10× with identical prompts and watch the throttle point arrive proportionally earlier (the reservation, made visible); on Anthropic, repeat a cached-prefix call and confirm from chapter 12's usage fields that reads bypass the ITPM meter while actual output drains the OTPM bucket. For Bedrock, reserve two ids and complete them in reverse order; assert each stored charge settles once, using the configured burndown when the completion does not override it. Reconcile your ledger's predicted deductions against the provider's reported counters over a 10-minute run — the residual is your ledger's error model.
