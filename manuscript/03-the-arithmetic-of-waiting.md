...
The consequence is the chapter's title fact: **cost does not scale linearly in context length.** Compare a 128k-token prompt to a 1M-token prompt. The dense part grows 8× (linear in N); the attention part grows with the square — 8² = 64× (exactly 64× in the chapter's binary counts, 131,072 → 1,048,576 being exactly 8×; 61× only if you read 128k and 1M as decimal round numbers — derived). A million tokens is not eight 128k prompts; it is roughly sixty-four of those 128k attention problems stitched into one grid. This is why TTFT balloons on giant prompts — the quadratic term, plus the KV allocation before the first token can exist — and why engines carve prefill into chunks with resources of their own (chapter 7).
...
> **Dated snapshot (mid-2026; exact rates change — the tiered *structure* is the durable fact).** Google's Gemini API prices input tokens in tiers with a step at the context boundary: $0.075 per 1M input tokens for prompts up to 128k, doubling to $0.15 above 128k (output $0.30 → $0.60); the tiered table was archived 2025-06-21 and verified still in place 2026-08-27 (Google AI pricing page). The mid-2026 Gemini 3.1 Pro table shows the same structure — $2.00 per 1M input up to 200k context, $4.00 above (ai.google.dev via Morph summary, 2026-08-27). And cached context reads are billed at a tenth of fresh input ($0.03 vs $0.30 per 1M tokens on Gemini 2.5 Flash, plus $1.00 per 1M tokens per hour of storage — the $0.075/$0.15 input rates at the top of this box are the archived 2025 tier) — direct market evidence that re-reading stored KV is an order of magnitude cheaper than recomputing a prompt (Gemini Developer API pricing, 2026-08-27).
