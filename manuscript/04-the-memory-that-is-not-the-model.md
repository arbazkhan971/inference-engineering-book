...
The mechanism, one rung up the ladder. A transformer (the network design behind every modern language model) is built from **layers** (stacked blocks — Llama 3.1 8B has 32 of them), and each layer's job is **attention**: when token number N+1 is being generated, it queries every earlier token — "how relevant are you to me?" — and blends their information by relevance. To be queryable, every token, in every layer, files two *vectors* when it passes through (a vector is just a list of numbers — the claim ticket and the bag are each one): a **key** (what to match on, like a claim ticket) and a **value** (the content to hand over, like the bag itself). Those keys and values never change once written — token 500's key is the same whether it is being read by token 501 or token 50,000.
...
Notice what kind of memory this is. It is not the model: it holds no learned knowledge, and an identical model restarted fresh has an empty cache. It is not your conversation text either: the raw tokens sit in ordinary RAM; the cache holds the *derived per-layer notes* — tens of thousands of times larger than the raw text. In this chapter's running example, one token is roughly 4 bytes of stored text (chapter 2's rule of thumb: one token ≈ four characters) but 128 KiB of notes — a ratio of 32,768× (derived: 131,072 ÷ 4; both inputs are rules of thumb, which is why the honest headline is "tens of thousands"). It is working state — the model's short-term memory of this conversation, in the exact shape the model will want to read it back. And it lives in HBM — the fast on-package memory at the top of chapter 3's pyramid — the scarcest, fastest memory in the building, because it is consulted at every layer of every decode step.
...
**Argument one: the window is an admission limit, enforced by arithmetic.** A provider's context window is not a statement about the model's attention span; it is a queue-policy constant: *max input tokens + max output tokens ≤ window*, checked at the door. The arithmetic shows through. OpenAI's GPT-5 family advertises a 400,000-token window with 128,000 reserved for output, so the maximum *input* is 272,000 — a gap developers meet as a surprise: the model page advertises 400k, the API rejects inputs above 272k (OpenAI model docs and developer forum reports, fetched 2026-08-27). The window is not the model's memory of you; it is the size of the desk the engine will allocate to your conversation — which is precisely why it is split and enforced before a single forward pass runs.
...
| Anthropic Claude (Opus/Sonnet tiers) | 1,000,000 tokens | No long-context surcharge on 4.6+ generations — 1M at standard price is the marketing; older sheets tiered (Sonnet 4.5: ≤200K at $3/$15 per MTok (million tokens) in/out, separate >200K rates) |
...
## 4.5 Four ways to shrink the coat: the architects' levers

> **ELI5:** Old hotels built one private coat rack per assistant — sixty-four racks, mostly holding near-duplicate bags. Four fixes exist. Fix one — share racks: sixty-four assistants, eight racks, bags passed hand to hand. Fix two — share a single rack: maximum savings, assistants start mishearing each other. Fix three — check photographs of the coats instead of the coats: near-perfect recall, a sliver of the shelf. Fix four — give most floors a whiteboard that only remembers the last thousand guests, and keep full racks on a few archive floors. Every modern model picked one of these — that is why the table in 4.3 varies.
...
**Multi-head attention (MHA)** is the original: every query head files its own K and V. For Llama-2-70B that meant 64 KV heads per layer — 2,560 KiB per token (≈2.6 MB), 20 GiB of cache for one 8k conversation (both exact from the formula: 2 × 80 × 64 × 128 × 2 bytes, then × 8,192; community formula guides and Llama 2 configs, 2026-08-27). Long context on that design was close to economically impossible.
...
**Grouped-query attention (GQA)** won the middle: keep all 64 query heads but file notes for only 8 KV heads, each group of eight queries sharing one set of keys and values (Ainslie et al., arXiv:2305.13245, 2023). Quality stays within evaluation noise of MHA at 8× compression. This is the 8-KV-heads pattern you met in every Llama/Qwen row of the table — the reason a 2024 70B caches 320 KiB where its 2023 grandparent cached 2,560 KiB: exactly one-eighth (2,560 ÷ 320). That change was a memory decision, not an intelligence decision. Nearly every frontier family (Llama, Mistral, Qwen, Gemma, Phi) landed on GQA (configs and technical reports summarized 2026-08-27).

**Multi-head latent attention (MLA)** attacks the notes instead of the head count. Instead of filing every head's full K and V, the model files one compressed *latent* per token per layer — a learned summary vector from which every head's notes are re-expanded at read time: 512 numbers plus a 64-number position-carrying key, 576 numbers total regardless of head count (DeepSeek-V3 Technical Report, arXiv:2412.19437, 2024). Work the idea once, two multiplications per side: a hypothetical 128-KV-head layer files 2 × 128 × 128 = 32,768 numbers per token — 64 KiB at two bytes each — while MLA files 576 numbers ≈ 1.1 KiB, the ~57× element reduction; run MLA's 576 × 2 bytes × 61 layers and you land on ≈68.6 KiB per token — DeepSeek-V3's row in the 4.3 table, sitting *below* Llama 3.1 8B's despite the model being vastly larger. That is roughly 60× in bytes end to end, and for long-agent-context serving it is the difference between viable and not: a 128K session costs 8.6 GiB instead of the ~30 GiB a GQA-8 redesign would charge, or ~512 GiB at full multi-head width (derived: 244 KiB and 4 MiB per token, respectively).
...
The ladder — the baseline plus the four ways — in one line each:
