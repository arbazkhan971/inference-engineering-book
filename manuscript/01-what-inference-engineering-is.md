...
| Token | The chopped-up word-piece the model reads and writes; the unit everything is priced and measured in | Arcade coins: the machine only takes tokens, never dollars |
...
Keep this table nearby. The rest of the chapter — and frankly the rest of the book — is these fourteen rows, given machinery and numbers.
...
The distinction that makes this a discipline rather than a buzzword: **prompt engineering optimizes what the model is told; inference engineering optimizes how the model runs.** The prompt engineer's lever is text. The inference engineer's levers are batch size, numerical precision, cache policy, engine choice, and fleet scheduling — and the canon is already written down. Chip Huyen's *AI Engineering* (2025) codifies the lever list: batching (especially continuous batching, which keeps GPUs busy by managing requests dynamically), quantization (shrinking numbers from 32-bit to 16-bit or less), KV caching (keeping the kitchen's copy of the ticket instead of re-reading the whole order — chapter 4's subject), and service-level optimization. The vLLM/PagedAttention paper (Kwon et al., SOSP 2023 — a top systems-engineering conference) supplied the canonical justification: existing systems wasted KV-cache memory on fragmentation and duplication, which capped batch size — fix the memory management and throughput (total work done per second across everyone sharing the kitchen) improves 2–4× at equal latency, with no change to the model or the prompt.
...
Bandwidth, not compute, sets the single-stream speed of generation. Every famous serving innovation is an attack on this wall from a different angle: batching (more requests per weight-streaming pass — Orca's iteration-level scheduling, OSDI 2022, SOSP's sister conference), KV-cache paging (fit more requests in memory — vLLM, 2–4× throughput), quantization (make the weights smaller — FP8, an 8-bit floating-point format, is "effectively lossless" across the Llama-3.1 family per 500,000+ evaluations by Databricks; full citation in Appendix E), speculative decoding (guess several tokens, verify in one pass). You will meet each one in its own chapter. The takeaway for now: those levers move speed, cost, and capacity *without touching the weights* — which is exactly why the serving layer deserves its own engineering discipline, and why "make the agent faster" almost never requires "make the model smarter."
...
Now zoom in on one order's journey: follow one streaming chat completion from your code to its last token.
...
7. **Decode.** One forward pass per token, batched across all running requests, each pass memory-bound. The KV cache grows by one entry per token — the dynamically growing memory that PagedAttention — chapter 6's subject — was built to manage (Kwon et al., 2023).
...
No codebase yet? Use any provider's five-line "first API call" quickstart as your streaming call — the timestamps are the lesson, not the surrounding app. Take one real streaming call — yours, or the quickstart — and wrap it with three timestamps: request sent, first content delta received, last delta received. Compute TTFT, mean inter-token gap, and end-to-end time. Check the identity: end-to-end ≈ TTFT + (N − 1) × mean gap, where N is the token count from the response's `usage` field. Ten lines of code; you have just built the seed of tinyengine's call tracer, and you will never again have to guess which hop is slow.
